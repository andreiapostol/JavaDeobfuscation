...
class QM9_Task(Sparse_Graph_Task):
    ...
    def make_task_output_model(self,
        ...
        placeholders['target_values'] = \
            tf.compat.v1.placeholder(dtype=tf.float32, shape=[len(self.params['task_ids']), None], name='target_values')
        ...
        for (internal_id, task_id) in enumerate(self.params['task_ids']):
            with tf.compat.v1.variable_scope("out_layer_task%i" % task_id):
                ...
                per_graph_outputs = tf.math.unsorted_segment_sum(data=per_node_gated_outputs,
                ...
                task_metrics['abs_err_task%i' % task_id] = tf.reduce_sum(input_tensor=tf.abs(per_graph_errors))
                tf.compat.v1.summary.scalar('mae_task%i' % task_id,
                                  task_metrics['abs_err_task%i' % task_id] / tf.cast(placeholders['num_graphs'], tf.float32))
                losses.append(tf.reduce_mean(input_tensor=0.5 * tf.square(per_graph_errors)))
        model_ops['task_metrics'] = task_metrics
        model_ops['task_metrics']['loss'] = tf.reduce_sum(input_tensor=losses)
