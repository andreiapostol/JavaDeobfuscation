...
class PPI_Task(Sparse_Graph_Task):
    ...
    def __load_data(self, data_dir: RichPath, data_fold: DataFold) -> List[GraphSample]:
        ...
        print("node_features shape is " + str(np.shape(node_to_features)) + " type " + str(type(node_to_features)))
        print("node_to_labels shape is " + str(np.shape(node_to_labels)) + " type " + str(type(node_to_labels)))
        print("node_to_graph_id shape is " + str(np.shape(node_to_graph_id)) + " type " + str(type(node_to_graph_id)))
        # print("graph_json_data nodes" + str(np.shape(graph_json_data["links"])))
        # print(node_to_labels[0])
        # print(node_to_graph_id[44000])
        # while True:
        #     x = None
        ...
        print("LENGTH IS ", len(final_graphs))
        return final_graphs
    ...
    def make_task_output_model(self,
        ...
        placeholders['target_labels'] = \
            tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.__num_labels], name='target_labels')
        placeholders['out_layer_dropout_keep_prob'] = \
            tf.compat.v1.placeholder(dtype=tf.float32, shape=[], name='out_layer_dropout_keep_prob')
        ...
        num_nodes_in_batch = tf.shape(input=placeholders['target_labels'])[0]

        f1_score = micro_f1(per_node_logits, placeholders['target_labels'])
        tf.compat.v1.summary.scalar("Micro F1", f1_score)
