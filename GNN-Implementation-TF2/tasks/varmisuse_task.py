...
class VarMisuse_Task(Sparse_Graph_Task):
    ...
    def make_task_output_model(self,
        ...
        correct_choices = tf.zeros([tf.shape(input=logits)[0]], dtype=tf.int32)
        if loss_function == 'max-likelihood':
            ...
            max_wrong_log_prob = tf.reduce_max(input_tensor=log_probs[:, 1:], axis=1)
            per_graph_loss = \
        ...
        prediction_is_correct = tf.equal(tf.argmax(input=tf.nn.softmax(logits), axis=1, output_type=tf.int32),
                                         correct_choices)
        accuracy = tf.reduce_mean(input_tensor=tf.cast(prediction_is_correct, tf.float32))

        tf.compat.v1.summary.scalar('accuracy', accuracy)
        model_ops['task_metrics'] = {
            'loss': tf.reduce_mean(input_tensor=per_graph_loss),
            'total_loss': tf.reduce_sum(input_tensor=per_graph_loss),
            'accuracy': accuracy,
            'num_correct_predictions': tf.reduce_sum(input_tensor=tf.cast(prediction_is_correct, tf.int32)),
        }
