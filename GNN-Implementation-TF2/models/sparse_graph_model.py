    ...
    def train(self, quiet: Optional[bool] = False, tf_summary_path: Optional[str] = None):
        ...
        with self.graph.as_default():
            if tf_summary_path is not None:
                ...
                # train_writer = tf.summary.create_file_writer(os.path.join(tf_summary_path, "train"), graph=self.graph)

                valid_writer = tf.compat.v1.summary.FileWriter(os.path.join(tf_summary_path, "valid"))
                # valid_writer = tf.summary.create_file_writer(os.path.join(tf_summary_path, "valid"))
            else:
